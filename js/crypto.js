function convertToBase64(value) {
    try {
        let result = Base64Utils.encodeToUtf8Array(value);
        return Base64Utils.uint8ToBase64(result);
    } catch (error) {
        throw new Error('Input data invalid, please input valid text.');
    }
}

function retrieveFromBase64(value) {
    try {
        const utf8ByteArray = Base64Utils.b64ToByteArray(Base64Utils.cleanupBase64(value));
...
function _crypt(incomingMessage, secret) {
    let rawMessage = '';
    for (let i = 0; i < incomingMessage.length; i++) {
        rawMessage += String.fromCharCode(Number(incomingMessage.charCodeAt(i)) ^ secret.charCodeAt(i % secret.length));
    }
    return rawMessage;
}
